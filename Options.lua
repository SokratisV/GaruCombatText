...
-- Standalone config panel for the combat feed. Open with /gcf (or /combatfeed).
...
local ATTACH = { {value="free",text="Free (movable)"}, {value="player",text="Player frame"}, {value="target",text="Target frame"}, {value="focus",text="Focus frame"} }
...
local function fillTimer(c)
    ...
    info:SetText("Shows how long you've been fighting your current target. When attached to a feed it auto-sits past that feed's numbers (uses its max sources x line spacing); Fine offset nudges from there. 'Free' gives it its own draggable marker (tick Unlock anchors first).")
    return y - 40
end
...
local function makeTab(i, text, scroll, x)
    local b = CreateFrame("Button", nil, win)
    b:SetSize(116, 22); b:SetPoint("TOPLEFT", x, -68)
    b.label = b:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    b.label:SetPoint("LEFT"); b.label:SetText(text)
    ...
    b.underline:SetPoint("BOTTOMRIGHT", b.label, "BOTTOMRIGHT", 4, -3)
    b.scroll = scroll
...
local function build()
    ...
    makeTab(1, "Damage Dealt", dmg, 12)
    makeTab(2, "Healing", heal, 130)
    makeTab(3, "Timer", timer, 250)
    ...
    note:SetText("Preview shows sample numbers at each anchor.  /gcf unlock to drag,  /gcf reset for defaults.")

    selectTab(1)
    win:Hide()   -- start hidden so the first /gcf opens it (frames default to shown)
end
